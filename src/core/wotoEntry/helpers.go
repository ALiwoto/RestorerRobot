...
//  default: panic(v)
//  }
func OldHandleNewUpdate(gUpdate tg.UpdateClass) {
...
func handleNewMessage(container *entryManager.WotoContainer) {
...
func handleNormalMessagesOld(message *tg.Message) {
	if message.Message != "" {
		if len(wotoValues.EntryMaster) == 0 {
			return
		}
		//go wotoValues.EntryMaster.TryToRun(message)
		//log.Println("got message: " + message.Message)
	}
}

func handleNormalMessages(container *entryManager.WotoContainer) {
	if container.Message != nil && container.Message.Message != "" {
		if len(wotoValues.EntryMaster) == 0 {
			return
		}
		wotoValues.EntryMaster.TryToRun(container)
		//log.Println("got message: " + message.Message)
	}
}

func handleServiceMessages(container *entryManager.WotoContainer) {
